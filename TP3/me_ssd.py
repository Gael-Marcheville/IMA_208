            ...
            for c in range(0,cur.shape[1], bcol):
                ...
                for dcol in range(-search, search, 1): # dcol = candidate displacement vector over the columns
                    for drow in range(-search, search, 1):# rcol = candidate displacement vector over the rows
                        r_trans = r + drow + extension
                        c_trans = c + dcol + extension
                    
                        R = ref_extended[r_trans:r_trans+brow, c_trans:c_trans+bcol] 
                        cost = np.sum(np.abs(R - B)**2)
            ...
            for c in range(0,cur.shape[1], bcol):
                ...
                for dcol in range(-search, search, 1): # dcol = candidate displacement vector over the columns
                    for drow in range(-search, search, 1):# rcol = candidate displacement vector over the rows
                        ...
                        R = ref_extended[r_trans:r_trans+brow, c_trans:c_trans+bcol] 
                        cost = np.sum(np.abs(R - B)**2) + lamb*((drow-pV[0])**2+(dcol-pV[1])**2)
